    ...
    stages {
        stage('Build') {
            steps {
                ...
                docker build -t $ECR_URL/alonit-polybot-dev:0.0.$BUILD_NUMBER .
                docker push $ECR_URL/alonit-polybot-dev:0.0.$BUILD_NUMBER
                '''
            }
        }
        ...
        stage('Trigger Release') {
            steps {
                build job: 'Release', wait: false, parameters: [
                    string(name: 'IMG_URL', value: "${ECR_URL}/alonit-polybot-dev:0.0.${BUILD_NUMBER}")
                ]
            }
        }
